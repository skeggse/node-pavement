...
function looper(iter, at, next, inner) {
  var body = 'for (' + iter + ' = 0, n = value.length; ' + iter + ' < n; ' +
    iter + '++) {\nif (value[' + iter + '] && ';
  // if (at === 'prop') {
  //   body += 'hasOwn.call(value[' + iter + '], ' + JSON.stringify(next) + ')';
  // } else {
  //   body += 'value[' + iter + '].length > ' + next;
  // }
  // body += ') {\n';
  body += inner;
  body += '}\n}\n';
    ...
    case 'match':
      ...
      if (token.next.type === 'look') {
        ...
        if (next.values.length === 1) {
          body += token.terminal ? 'return ' : 'value = ';
          body += 'value[i]' + getter(nextValue.value) + ';\n';
        } else {
          body += 'value = value[i];\n';
          if (token.terminal) {
            body += 'return ' + fullGetter('value', utils.pluck(next.values, 'value')) + ';\n';
          }
        }
        if (!token.terminal) {
          body += 'break;\n';
...
var token = tokenize(['eight', -1, -1, 'one']);
var fn = compile(token.varOpts, token.tokens);
console.log(fn + '');
console.log(fn(), fn({}), fn({eight: null}), fn({eight: []}), fn({eight: [[]]}), fn({eight: [[{}]]}), fn({eight: [[{one: 0}]]}), fn({eight: [,[{one: 0}]]}), fn({eight: [[,{one: 0}]]}), fn({eight: [,[,{one: 0}]]}));
